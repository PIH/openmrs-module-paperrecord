...
public class DefaultZplPaperRecordLabelTemplate implements PaperRecordLabelTemplate {
    ...
    @Override
    public String generateLabel(Patient patient, String paperRecordIdentifier, Map<String, List<String>> externalIdentifiers) {
        ...
        if(externalIdentifiers!=null && externalIdentifiers.size()>0){
            for (String s : externalIdentifiers.keySet()) {
                List<String> ids = externalIdentifiers.get(s);
                if(ids!=null && ids.size()>0){
                    for (String id : ids) {
                        data.append("^FO680," + verticalPosition + "^FB520,1,0,R,0^AUN^FD" + messageSourceService.getMessage("ui.i18n.PatientIdentifierType.name." + s)
                                + " " + id + "^FS");
        ...
        data.append("^FO780," + verticalPosition + "^ATN^BY4^BCN,150,N^FD" + primaryIdentifier.getIdentifier() + "^FS");    // print barcode & identifier
